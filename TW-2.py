...
import joblib
from sklearn.model_selection import TimeSeriesSplit  # 💡 引入時間序列交叉驗證模組
...
# 宣告時間序列驗證器，指定每個測試集長度為 predict_step (20天)
...
for fold, (train_index, test_index) in enumerate(tscv.split(X)):
    # 確保初始訓練集達到指定的天數天窗，若 TimeSeriesSplit 前期切出的訓練集太小則跳過
    if len(train_index) < train_window:
        continue

    # ------------------------------------------------------
    # 模式 A：擴展窗口 (Expanding Window) -> 訓練集會隨時間越滾越大 (原本的邏輯)
    X_train_roll = X.iloc[train_index]
    y_train_roll = y.iloc[train_index]

    # 模式 B：固定滑動窗口 (Sliding Window) -> 只看最近 1000 天
    # 若想改用固定 1000 天訓練，請自由將下方兩行程式碼的「#」註解拔除：
    # X_train_roll = X.iloc[train_index[-train_window:]]
    # y_train_roll = y.iloc[train_index[-train_window:]]
    # ------------------------------------------------------
...
plt.plot(strategy_cumulative, label="Strategy (TimeSeriesSplit策略)", color='red', linewidth=2)
plt.title(f"{stock_code} 機器學習-TimeSeriesSplit交叉驗證回測績效")
plt.ylabel("累積報酬率")
plt.xlabel("驗證交易天數")
plt.legend()
