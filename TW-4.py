...
import joblib
from datetime import datetime  # 📌 引入 datetime 模組來自動取得最新日期
...
# 📌 自動取得今天的日期字串 (格式如: '2026-07-09')
today_str = datetime.today().strftime('%Y-%m-%d')

# 1. 下載資料 (預設先抓上市，end 改為動態的 today_str)
df = yf.download(stock_code, start="2015-01-01", end=today_str)

# 防呆機制：如果上市抓不到，自動切換到上櫃市場 (.TWO)
if df.empty:
    alternative_code = f"{stock_input}.TWO"
    print(f"⚠️ 在上市市場找不到該股票，正在嘗試上櫃市場 {alternative_code}...")
    df = yf.download(alternative_code, start="2015-01-01", end=today_str)
    if not df.empty:
...
# ==========================================================
# 🔥 【將目標設定為未來 10 個交易日的報酬率】(對應迴圈 range(1, 11))
# ==========================================================
target_cols = []
...
if X_today.isnull().values.any():
    ...
    # 同時預測未來 10 個交易日的報酬率 (配合前面 shift 計算)
    ...
    # 雖然預測了 10 天，但下方報表只印出前 5 天（一週）的軌跡
    for day_idx, r_pred in enumerate(weekly_pred_returns[:5], start=1):
